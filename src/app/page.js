//Hompage is Server Component => client
'use client'
...
export default function Page() {
  ...
  const [data,setData] = useState({recipe:[], custom:[], material:[]});
  useEffect(()=>{
    const getData = async() => {
      const recipePromise = await fetch('/api/recipedata',{ method:"POST",headers:{'Content-Type':'application/json'},
      body:JSON.stringify({page_size:6,filter:{"property":"type","select":{"equals":"Cocktell"}}})});
      const recipeData = await recipePromise.json();
      const customPromise = await fetch('/api/customdata',{ method:"POST",headers:{'Content-Type':'application/json'},
      body:JSON.stringify({page_size:3,filter:{"property":"type","select":{"equals":"Custom"}}})});
      const customData = await customPromise.json();
      const materialPromise = await fetch('/api/materialdata',{ method:"POST",headers:{'Content-Type':'application/json'},
      body:JSON.stringify({})});
      const materialData = await materialPromise.json();
      setData({recipe:recipeData, custom:customData, material:materialData});
      console.log(data);
    }; getData();
  ...
  return (
    <div className='home_page'> {/* Home Component : Client Component */}
        ...
        <RecipePreview recipeData={data.recipe} />
        <div className="mt-40 mb-40 border-b border-gray-300"></div>
        <CustomPreview customData={data.custom} />
        <div className="mt-40 mb-40 border-b border-gray-300"></div>
        <MyBagPreview materialData={data.material} />
        <div className="mt-40 mb-40 border-b border-gray-300"></div>
